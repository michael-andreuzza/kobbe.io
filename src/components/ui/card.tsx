...
function Card({
  ...
  return (
    <div
      ...
      className={cn(
        "group/card bg-card text-card-foreground flex flex-col gap-4 overflow-hidden rounded-md py-4 text-sm has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-3 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-md *:[img:last-child]:rounded-b-xl",
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "bg-card flex items-center rounded-b-sm p-4 group-data-[size=sm]/card:p-3",
        className,
      )}
      {...props}
    />
