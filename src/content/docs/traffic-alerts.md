...
Traffic alerts email you when a site's recent pageview volume changes sharply compared with its recent baseline. They are off by default and can be enabled from Site settings → Reports & alerts.
...
Traffic alerts are included on Starter plans and higher on the [pricing section](/#pricing). They are opt-in per site. You can disable them any time from site settings.
