...
Drop pageviews and custom events **before** they count toward quotas or appear in the dashboard. This is separate from [bot filtering](/docs/script-options#bot-filtering-and-exclusions)—bots are handled first; visit filters are rules you set per site. The **Bots** tab on the dashboard's Sources card shows automated traffic Kobbe already filtered out.
